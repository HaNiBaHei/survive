...
void DefaultEditorMode::updateInput(const float& dt)
{
	...
	else if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key(this->editorStateData->keybinds->at("INCREASE_TYPE"))) && this->getKeytime()) // INCREASE TYPE //
	{
		++this->type;
	}
	else if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key(this->editorStateData->keybinds->at("DECREASE_TYPE"))) && this->getKeytime()) // DECREASE TYPE //
	{
		if (this->type > 0)
			--this->type;
	}
