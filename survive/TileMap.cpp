...
void TileMap::updateCollision(Entity* entity)
{
	...
	else if (entity->getPosition().x + entity->getGlobalBound().width > this->maxSizeWorldF.x)
		entity->setPosition(this->maxSizeWorldF.x, entity->getPosition().y);

	if (entity->getPosition().y < 0.f)
		entity->setPosition(entity->getPosition().x, 0.f);
	else if (entity->getPosition().y + entity->getGlobalBound().height > this->maxSizeWorldF.y)
